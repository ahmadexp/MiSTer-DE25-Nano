#!/usr/bin/env bash
set -euo pipefail

target_root=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
remote=${DE25_BUILD_HOST:?Set DE25_BUILD_HOST to the Quartus SSH target}
remote_root=${DE25_REMOTE_ROOT:-MiSTer-DE25-Nano}

ssh "$remote" "mkdir -p '$remote_root/de25-nano'"
rsync -az --delete \
    --exclude artifacts \
    --exclude ip/ip \
    --exclude ip/pc110_pll \
    --exclude ip/pc110_pll.qsys \
    --exclude quartus/db \
    --exclude quartus/dni \
    --exclude quartus/incremental_db \
    --exclude quartus/output_files \
    --exclude quartus/output_files_clocks \
    --exclude quartus/qdb \
    "$target_root/" "$remote:$remote_root/de25-nano/"

ssh -t "$remote" "cd '$remote_root/de25-nano' && chmod +x scripts/*.sh && scripts/build-clocks.sh"

mkdir -p "$target_root/artifacts"
rsync -az \
    "$remote:$remote_root/de25-nano/quartus/output_files_clocks/DE25_PC110_CLOCKS.sof" \
    "$target_root/artifacts/"
