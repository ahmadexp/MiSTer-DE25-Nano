...
project=${DE25_SMS_PROJECT:-DE25_MISTER_SMS}
output_directory=${DE25_SMS_OUTPUT_DIRECTORY:-output_files_sms}
output_rbf=${DE25_SMS_OUTPUT_RBF:-$platform_root/artifacts/sms/SMS_20260814.rbf}
...
if command -v git >/dev/null 2>&1; then
    "$platform_root/scripts/apply-sms-patches.sh"
fi
...
generate_and_build() {
    ...
    printf '`define BUILD_DATE "260814"\n' > \
        "$platform_root/upstream/cores/SMS/build_id.v"
    ghrd_root=$("$platform_root/scripts/prepare-ghrd-worktree.sh" "$project")
    cd "$platform_root/ip"
    rm -rf mister_pll sms_core_pll_cal \
        ip/mister_pll ip/sms_core_pll_cal \
        "$ghrd_root/mister_hps" "$ghrd_root/ip/mister_hps"
    rm -f mister_pll.qsys sms_core_pll_cal.qsys \
    ...
    qsys-script --quartus-project="$platform_root/quartus/$project" \
        --rev="$project" --script=create_sms_core_pll_cal.tcl
    qsys-script --quartus-project="$platform_root/quartus/$project" \
    ...
    quartus_ipgenerate "$project" -c "$project" --run_default_mode_op \
        --parallel=off
    "$platform_root/scripts/fix-emif-calibration-ip.sh" sms_core_pll_cal
...
[[ -z ${DE25_SMS_PROJECT:-} ]] || docker_args+=( -e "DE25_SMS_PROJECT=$DE25_SMS_PROJECT" )
[[ -z ${DE25_SMS_OUTPUT_DIRECTORY:-} ]] || docker_args+=( -e "DE25_SMS_OUTPUT_DIRECTORY=$DE25_SMS_OUTPUT_DIRECTORY" )
...
if [[ -n ${DE25_SMS_OUTPUT_RBF:-} ]]; then
    sms_output_rbf=$("$platform_root/scripts/docker-workspace-path.sh" \
        "$workspace_root" "$DE25_SMS_OUTPUT_RBF")
    docker_args+=( -e "DE25_SMS_OUTPUT_RBF=$sms_output_rbf" )
fi

exec docker "${docker_args[@]}" "$image" bash -lc \
    '/work/MiSTer-DE25-Nano/mister-de25/scripts/build-sms.sh'
