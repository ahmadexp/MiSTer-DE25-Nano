...
project=DE25_MISTER_SNES
output_rbf=${DE25_SNES_OUTPUT_RBF:-$platform_root/artifacts/snes/SNES_20260812.rbf}
...
if command -v git >/dev/null 2>&1; then
    "$platform_root/scripts/apply-snes-patches.sh"
fi
...
generate_and_build() {
    ...
    printf '`define BUILD_DATE "260812"\n' > \
        "$platform_root/upstream/cores/SNES/build_id.v"
    ghrd_root=$("$platform_root/scripts/prepare-ghrd-worktree.sh" "$project")
    cd "$platform_root/ip"
    rm -rf mister_pll snes_core_pll_cal \
        ip/mister_pll ip/snes_core_pll_cal \
        "$ghrd_root/mister_hps" "$ghrd_root/ip/mister_hps"
    rm -f mister_pll.qsys snes_core_pll_cal.qsys \
    ...
    qsys-script --quartus-project="$platform_root/quartus/$project" \
        --rev="$project" --script=create_snes_core_pll_cal.tcl
    qsys-script --quartus-project="$platform_root/quartus/$project" \
    ...
    "$platform_root/scripts/fix-emif-calibration-ip.sh" snes_core_pll_cal
...
if [[ -n ${DE25_SNES_OUTPUT_RBF:-} ]]; then
    snes_output_rbf=$("$platform_root/scripts/docker-workspace-path.sh" \
        "$workspace_root" "$DE25_SNES_OUTPUT_RBF")
    docker_args+=( -e "DE25_SNES_OUTPUT_RBF=$snes_output_rbf" )
fi

exec docker "${docker_args[@]}" "$image" bash -lc \
    '/work/MiSTer-DE25-Nano/mister-de25/scripts/build-snes.sh'
