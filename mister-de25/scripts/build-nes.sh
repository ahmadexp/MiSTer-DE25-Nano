...
if command -v git >/dev/null 2>&1; then
    "$platform_root/scripts/apply-nes-patches.sh"
fi
...
generate_and_build() {
    ...
    cd "$platform_root/ip"
    rm -rf mister_pll nes_core_pll_cal \
        ip/mister_pll ip/nes_core_pll_cal \
    ...
    # The HPS I/O signature must come from the freshly generated common
    # platform, never from an incremental database left by an older build.
    quartus_sh --clean -c "$project" "$project"
    # Serial generation avoids intermittent local worker disconnects in the
    # EMIF package generator and gives this release build deterministic logs.
...
if [[ -n ${DE25_NES_OUTPUT_RBF:-} ]]; then
    nes_output_rbf=$("$platform_root/scripts/docker-workspace-path.sh" \
        "$workspace_root" "$DE25_NES_OUTPUT_RBF")
    docker_args+=( -e "DE25_NES_OUTPUT_RBF=$nes_output_rbf" )
fi
