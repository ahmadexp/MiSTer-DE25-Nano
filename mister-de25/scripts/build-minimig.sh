...
project=DE25_MISTER_MINIMIG
output_rbf=${DE25_MINIMIG_OUTPUT_RBF:-$platform_root/artifacts/minimig/Minimig_20260812.rbf}
...
if command -v git >/dev/null 2>&1; then
    "$platform_root/scripts/apply-minimig-patches.sh"
fi
...
generate_and_build() {
    ...
    printf '`define BUILD_DATE "260812"\n' > \
        "$platform_root/upstream/cores/Minimig/build_id.v"
    ghrd_root=$("$platform_root/scripts/prepare-ghrd-worktree.sh" "$project")
    cd "$platform_root/ip"
    rm -rf mister_pll minimig_core_pll_cal \
        ip/mister_pll ip/minimig_core_pll_cal \
        "$ghrd_root/mister_hps" "$ghrd_root/ip/mister_hps"
    rm -f mister_pll.qsys minimig_core_pll_cal.qsys \
    ...
    qsys-script --quartus-project="$platform_root/quartus/$project" \
        --rev="$project" --script=create_minimig_core_pll_cal.tcl
    qsys-script --quartus-project="$platform_root/quartus/$project" \
    ...
    "$platform_root/scripts/fix-emif-calibration-ip.sh" minimig_core_pll_cal
    ...
    ../scripts/check-timing-summary.sh \
        "output_files_minimig/$project.sta.summary"
    quartus_sta -t ../scripts/report-sdram-timing.tcl "$project"
    "$platform_root/scripts/make-hps-first-rbf.sh" \
        "output_files_minimig/$project.sof" "$output_rbf" "$hps_bootloader"
}
...
if [[ -n ${DE25_MINIMIG_OUTPUT_RBF:-} ]]; then
    minimig_output_rbf=$("$platform_root/scripts/docker-workspace-path.sh" \
        "$workspace_root" "$DE25_MINIMIG_OUTPUT_RBF")
    docker_args+=( -e "DE25_MINIMIG_OUTPUT_RBF=$minimig_output_rbf" )
fi

exec docker "${docker_args[@]}" "$image" bash -lc \
    '/work/MiSTer-DE25-Nano/mister-de25/scripts/build-minimig.sh'
